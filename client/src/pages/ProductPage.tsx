...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    setProduct(product);
    setRelatedProducts(dummyProducts.filter((p) => p.category === product?.category && p._id !== product?.id));
    setLoading(false);
  }, [id, navigate]);
          ...
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="relative flex-center p-8 md:p-12 min-h-80 md:min-h-120">
              ...
              <div className="absolute top-5 left-5 flex flex-wrap gap-1.5">
                {product.isOrganic && <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-app-green text-white rounded-full"><LeafIcon className="w-3 h-3" /> Organic</span>}
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              ...
              <div>
                {product.stock > 0 ? (
                  <span className="text-sm text-app-success font-medium">✔️ In Stock ({product.stock} available)</span>
                ): (
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-12 mb-44">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to={`/products?category=${product.category}`} className="flex items-center gap-1 text-app-orange hover:text-app-orange-dark bg-app-cream text-sm font-semibold transition-colors">
